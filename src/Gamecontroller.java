...
private final Color[] guesses = new Color[GameController.CODE_LENGTH];
...
public VBox createGameLayout() {
    ...
    for (int i = 0; i < GameController.CODE_LENGTH; i++) {
...
private void generateSecretCode() {
    secretCode = new ArrayList<>(seconds);
    Collections.shuffle(secretCode);
    secretCode = secretCode.subList(0, GameController.CODE_LENGTH);
}
...
private void submitGuess() {
    if (attempts < GameController.MAX_ATTEMPTS) {
        ...
        if (isCorrectGuess(guesses)) {
            ...
            endGame(true);
        } else if (attempts == GameController.MAX_ATTEMPTS) {
